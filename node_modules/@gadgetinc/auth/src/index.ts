...
export type Providers<UserInput> = {
  ...
  scopes?: string[];
  /**
   * The default set of roles to assign to the `Session` when a successful login has occurred.
   */
      ...
      >(this: FastifyInstance, request: RouteContext, reply: FastifyReply) {
        ...
        if (userRecord) {
          ...
          const result = await options.api.mutate(
            ...
            `,
            {
              user: googleResultToUser(claims),
              trigger: {
          ...
          applicationSession?.set(
            "user",
            result.internal[config.userModel[googleSignupTrigger].triggerAction][config.userModel.apiIdentifier].id
          );
        }
