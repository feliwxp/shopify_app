...
exports.Auth = (0, fastify_plugin_1.default)((fastify, options, done) => {
    if (options.providers.length > 0) {
        ...
        fastify.get(callbackPath, async function (request, reply) {
            ...
            if (userRecord) {
              ...
              mutation TriggerUpdateUser($id: GadgetID!, $user: ${config.userModel[googleSigninTrigger].inputType}, $trigger: JSONObject, $verify: Boolean) {
                internal {
                  ${config.userModel[googleSigninTrigger].triggerAction}(id: $id, ${config.userModel.apiIdentifier}: $user, trigger: $trigger, verifyTriggerExists: $verify) {
                    success
                    errors {
                      message
                      code
            ...
            `, {
                ...
                const result = await options.api.mutate(`
              mutation TriggerCreateUser($user: ${config.userModel[googleSignupTrigger].inputType}, $trigger: JSONObject, $verify: Boolean) {
                internal {
                  ${config.userModel[googleSignupTrigger].triggerAction}(${config.userModel.apiIdentifier}: $user, trigger: $trigger, verifyTriggerExists: $verify) {
                    success
            ...
            `, {
                    user: (0, exports.googleResultToUser)(claims),
                    trigger: {
                ...
                applicationSession?.set("user", result.internal[config.userModel[googleSignupTrigger].triggerAction][config.userModel.apiIdentifier].id);
            }
    ...
    fastify.post("/auth/signout", async function (request, reply) {
        const applicationSession = getSessionFromRequest(request);
